...
export const messages = {
  ...
  profile: {
    ...
    contextMenu: {
      notLinked: {
        title: "Aucun pseudo Minecraft trouvé",
        description: (user: string, tried: string) =>
          `Impossible d'associer **${user}** à un pseudo Minecraft automatiquement.\nCandidats essayés : ${tried}\nUtilise \`/profile joueur:<pseudo>\` si besoin.`,
      },
      matched: (candidate: string) => `Ouvert via le pseudo détecté \`${candidate}\`.`,
      none: "aucun",
    },
    pagination: {
      locked: {
        title: "Panneau verrouillé",
        description: "Seule la personne qui a ouvert ce profil peut changer de page.",
      },
    },
    title: (player: string) => `Profil de ${player}`,
    description: (title: string, summary: string) => `**${title}**\n${summary}`,
    noBadges: "Aucun badge majeur débloqué pour l'instant.",
    pages: {
    ...
    legend: {
      noBadges: "Aucun badge rare pour le moment. L'aventure ne fait que commencer.",
      lines: {
        playTime: (value: string) => `A déjà passé **${value}** dans le monde.`,
        distance: (value: string) => `A parcouru **${value}** au total.`,
        mobs: (value: string) => `A vaincu **${value}** créatures hostiles.`,
        deaths: (value: string) => `Est tombé **${value}** fois avant de repartir.`,
      },
    },
